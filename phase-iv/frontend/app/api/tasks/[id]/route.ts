 ...
 * Handles: GET (get task), PUT (update task), DELETE (delete task), PATCH (toggle complete)
 *
...
// Default demo user (auth bypassed)
...
async function createToken(userId: string | number, email?: string): Promise<string> {
  const secret = new TextEncoder().encode(process.env.BETTER_AUTH_SECRET || 'demo-secret-for-hackathon-phase-iv-min32');
