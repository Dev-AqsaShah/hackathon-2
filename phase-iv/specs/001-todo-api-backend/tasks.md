...
- [X] T001 Verify backend dependencies installed (requirements.txt: FastAPI, SQLModel, python-jose, asyncpg, alembic)
- [X] T002 [P] Verify database connection to Neon PostgreSQL (test DATABASE_URL from .env)
- [X] T003 [P] Verify JWT authentication infrastructure exists (core/security.py, api/deps.py)
- [X] T004 Review existing User model in backend/app/models/user.py for compatibility
...
- [X] T005 Review existing Todo model in backend/app/models/todo.py against spec requirements
- [X] T006 Create Alembic migration 002 to rename `todos` table to `tasks` in backend/alembic/versions/002_rename_todos_to_tasks.py
- [X] T007 Update model class name from Todo to Task in backend/app/models/todo.py (rename file to task.py)
- [X] T008 Verify Task model fields match spec: id, title(1000), description(5000), completed, owner_id, created_at, updated_at
- [X] T009 Add check constraint for non-empty title in migration 002
- [X] T010 Verify index on owner_id exists in migration 002
- [X] T011 Run migration: `alembic upgrade head` and verify tasks table created

### Request/Response Schemas

- [X] T012 [P] Create TaskCreate schema in backend/app/schemas/task.py with title (required, 1-1000 chars) and description (optional, 0-5000 chars)
- [X] T013 [P] Create TaskUpdate schema in backend/app/schemas/task.py with optional title and description
- [X] T014 [P] Create TaskResponse schema in backend/app/schemas/task.py with all fields (id, title, description, completed, owner_id, created_at, updated_at)
- [X] T015 Add Pydantic field validators to TaskCreate: min_length=1, max_length=1000 for title
- [X] T016 Add Config examples to all schemas for OpenAPI documentation

### Service Layer Foundation

- [X] T017 Create services directory: backend/app/services/
- [X] T018 Create task_service.py in backend/app/services/ with async function stubs
- [X] T019 Implement validate_ownership(task: Task, user_id: int) -> None in task_service.py (raises 403 if mismatch)
...
- [X] T020 [US1] Implement get_user_tasks(session, user_id) in backend/app/services/task_service.py to query tasks filtered by owner_id
- [X] T021 [US1] Create tasks router in backend/app/api/routes/tasks.py with APIRouter()
- [X] T022 [US1] Implement GET /api/{user_id}/tasks endpoint in backend/app/api/routes/tasks.py
- [X] T023 [US1] Inject CurrentUser dependency in GET endpoint (from api/deps.py)
- [X] T024 [US1] Add path user_id validation: raise 403 if path user_id != JWT user_id
- [X] T025 [US1] Call task_service.get_user_tasks() and return List[TaskResponse]
- [X] T026 [US1] Add OpenAPI documentation to GET endpoint (summary, description, responses)
- [X] T027 [US1] Register tasks router in backend/app/main.py with prefix "/api/{user_id}"
...
- [X] T028 [P] [US2] Implement create_task(session, data: TaskCreate, owner_id: int) in backend/app/services/task_service.py
- [X] T029 [P] [US2] Set created_at and updated_at timestamps automatically in create_task()
- [X] T030 [P] [US2] Set completed=False by default in create_task()
- [X] T031 [US2] Implement POST /api/{user_id}/tasks endpoint in backend/app/api/routes/tasks.py
- [X] T032 [US2] Inject CurrentUser dependency and validate path user_id matches JWT
- [X] T033 [US2] Accept TaskCreate request body and validate with Pydantic
- [X] T034 [US2] Call task_service.create_task() with authenticated owner_id
- [X] T035 [US2] Return 201 Created status code with TaskResponse body
- [X] T036 [US2] Add OpenAPI documentation to POST endpoint
...
- [X] T037 [P] [US5] Implement get_task_by_id(session, task_id: int, user_id: int) in backend/app/services/task_service.py
- [X] T038 [P] [US5] Add ownership validation in get_task_by_id: raise 404 if not found, 403 if wrong owner
- [X] T039 [US5] Implement GET /api/{user_id}/tasks/{id} endpoint in backend/app/api/routes/tasks.py
- [X] T040 [US5] Inject CurrentUser dependency and validate path user_id
- [X] T041 [US5] Call task_service.get_task_by_id() with task_id and authenticated user_id
- [X] T042 [US5] Return 200 OK with TaskResponse body
- [X] T043 [US5] Add OpenAPI documentation to GET single task endpoint
...
- [X] T044 [P] [US3] Implement update_task(session, task_id, data: TaskUpdate, user_id) in backend/app/services/task_service.py
- [X] T045 [P] [US3] Auto-update updated_at timestamp in update_task()
- [X] T046 [P] [US3] Handle partial updates (title-only, description-only, or both) in update_task()
- [X] T047 [P] [US3] Implement toggle_task_completion(session, task_id, user_id) in backend/app/services/task_service.py
- [X] T048 [P] [US3] Flip completed boolean and update updated_at in toggle_task_completion()
- [X] T049 [US3] Implement PUT /api/{user_id}/tasks/{id} endpoint in backend/app/api/routes/tasks.py
- [X] T050 [US3] Inject CurrentUser, validate path user_id, accept TaskUpdate body
- [X] T051 [US3] Call task_service.update_task() and return 200 with TaskResponse
- [X] T052 [US3] Implement PATCH /api/{user_id}/tasks/{id}/complete endpoint in backend/app/api/routes/tasks.py
- [X] T053 [US3] PATCH endpoint takes no request body, only toggles state
- [X] T054 [US3] Call task_service.toggle_task_completion() and return 200 with TaskResponse
- [X] T055 [US3] Add OpenAPI documentation to both PUT and PATCH endpoints
...
- [X] T056 [US4] Implement delete_task(session, task_id, user_id) in backend/app/services/task_service.py
- [X] T057 [US4] Verify ownership before deletion in delete_task() (raise 403/404)
- [X] T058 [US4] Implement DELETE /api/{user_id}/tasks/{id} endpoint in backend/app/api/routes/tasks.py
- [X] T059 [US4] Inject CurrentUser and validate path user_id
- [X] T060 [US4] Call task_service.delete_task()
- [X] T061 [US4] Return 204 No Content status (no response body)
- [X] T062 [US4] Add OpenAPI documentation to DELETE endpoint
