...
import { TaskCreateInput, Task, Priority } from '@/types/task';
...
export function TaskForm({ initialData, userId, mode }: TaskFormProps) {
  ...
  const [dueDate, setDueDate] = useState(
    initialData?.due_date ? initialData.due_date.slice(0, 16) : ''
  );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                disabled={loading}
                className="w-full h-12 px-4 rounded-xl border bg-dark-800/50 backdrop-blur-sm text-dark-100
                           border-dark-700 focus:ring-2 focus:ring-accent-500/50 focus:border-accent-500/50
            ...
            {/* Due Date */}
            <div>
              <label htmlFor="due_date" className="block text-sm font-medium text-dark-300 mb-2">
                Due Date <span className="text-dark-500">(optional)</span>
              </label>
              <input
                id="due_date"
                type="datetime-local"
